const { MessageButton, MessageActionRow, ActionRowBuilder, ButtonBuilder } = require('discord.js');
const { Produto, ProdutoEstoque, Carrinho } = require('../database/cart');
...
const iniciarCompra = async (interaction, categoriaCarrinho) => {
    ...
    if (!produtoEscolhido) return interaction.reply({ content: `${interaction.user}, no momento não há estoque do produto \`${nome}\``, ephemeral: true });
    
    await interaction.deferUpdate();
    
    /** @type {TextChannel} */
    const carrinhoCanal = await categoriaCarrinho.fetch(c => c.topic === interaction.user.id) && await criarCarrinho(categoriaCarrinho, interaction);
    ...
    let msgProduto = interaction.message

    // Impedir adicionar o mesmo produto no carrin
